...
{
    public static Dictionary<string, SerializedProperty> GetPhysicsShapeProperties(
    ...
    {
        SerializedProperty property = default;

        foreach (var item in newProperties)
        {
            if (!oldProperties.TryGetValue(item.Key, out var oldItem)) continue;
            ...
            if (oldItem.arraySize > 0)
            {
                ...
                for (var index = 0; index < newItem.arraySize; ++index)
                {
                    ...
                    for (var pi = 0; pi < newShape.arraySize; ++pi)
                    {
                ...
                if (property == default) property = newItem;
            }
        }
